...
import jvmlabs.build.version.service.GitService;
import jvmlabs.build.version.service.TreeState;
...
import static jvmlabs.build.version.Preconditions.checkNotNull;
import static jvmlabs.build.version.Preconditions.checkState;
...
public final class SemanticVersionPlugin implements Plugin<Project> {
    ...
    private static final GradleVersion minSupportedGradleVersion = GradleVersion.version("8.0");
    ...
    public void apply(Project project) {
        ...
        final var version = makeVersionProvider(project, extension, metadata);
        project.allprojects(it -> {
            it.setVersion(ExtendedVersionStringProvider.wrap(version));
            final var versionInfo = it.getTasks().register("versionInfo", VersionInfoTask.class, task -> {
                ...
                task.getProjectVersion().set(it.getVersion().toString());
    ...
    private Provider<SemanticVersion> makeVersionProvider(
        ...
        final var provider = new SemanticVersionProviderBuilder(project)
                ...
                .build();
        extension.getValue().value(provider).finalizeValueOnRead();
    ...
    private Provider<String> makeMetadataProvider(
        ...
        final var property = project.getObjects().property(String.class).value(provider);
